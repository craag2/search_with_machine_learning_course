...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        #get feature names from ltr_featureset.json
        with open('/workspace/search_with_machine_learning_course/week1/conf/ltr_featureset.json') as json_file:
            # print('chirag')
            ltr_features = json.load(json_file)
            features = ltr_features['featureset']['features']
            # print(features)
        ...
        if 'hits' in hits:
            ...
            for hit in hits:
                ...
                # print(type(hit['_source']['sku']))

                ltr_features= hit['fields']['_ltrlog'][0]
                # print(ltr_features[0])

                for feature in ltr_features['log_entry']:
                    # print(feature)
                    name = feature['name']
                    if 'value' in feature:
                        value = feature['value']
